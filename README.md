...
- ✅ **Workers' pay**: the CEO pay ratio (median employee vs CEO) for 2,253 US companies from their proxy statements, and
  salaries by job title — 678 companies from 286,961 H-1B wage filings (`--salaries`) and 550 companies from the pay
  ranges in their own job ads (`--postings`: 26,719 job titles, including hourly and front-line jobs), 1,039 US companies in all
- ✅ **Search engines**: every company, salaries, executive and search page is rendered on the server with its content; the API is
...
| `GET /api/v1/companies/{ticker}/salaries` | Salaries by job title, company-wide and per city, from the company's job ads and its US H-1B wage filings |
