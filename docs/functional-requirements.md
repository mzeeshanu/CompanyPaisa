...
- **Search addresses:** the search on screen is always in the address bar, so it can be shared, bookmarked and reloaded: `/near/{place}` for companies, `/near/{place}/executives` for executives, plus only the settings that differ from the defaults — `radius`, `sector`, `hq=1`, `sort`, and for executives `q` (name/title filter) and `former=1`. Map / List and the theme stay personal preferences (cookie), not part of the address. `{place}` is the postcode without spaces (`84043`, `SW1A1AA`, `M5J2J2`), European / Australian / NZ codes with their country (`FR-75008`, `NL-1012AB`, `AU-2000`), or `me` for "my location" — the visitor's coordinates never go in the address. Opening a `/near/me` link uses the location only if the browser already allows it; otherwise the location popup asks. An unknown place opens the popup with "We couldn't find …". Changing filters updates the address without adding Back-button steps.
...
- **Layout:** two columns on wide screens — left: **Earnings**, **At a glance**, **Similar companies nearby**, **Locations**; right: **Executive pay** (it is the tallest card). One column on narrow screens in the order Earnings, At a glance, Executive pay, Similar companies, Locations.
...
- Footer: data source note (SEC filings / UK or European annual reports). Unknown ticker: "Company not found" with a way back to the search.
